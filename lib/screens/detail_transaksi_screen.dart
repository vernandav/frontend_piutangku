...
class _DetailTransaksiScreenState extends State<DetailTransaksiScreen> {
  ...
  Future<void> fetchDetail() async {
    ...
    final res = await http.get(
      Uri.parse('http://localhost:3000/api/transaksi/detail/${widget.transaksiId}'),
  ...
  Future<void> deleteCicilan(int id) async {
    ...
    final res = await http.delete(
      Uri.parse('http://localhost:3000/api/cicilan/delete/$id'),
      headers: {'Authorization': 'Bearer $token'},
    );
    ...
    if (res.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Gagal menghapus cicilan')),
      );
    }
  }
                            ...
                            Column(
                              children: [
                                ...
                                if (transaksi!.containsKey('metode_cicilan'))
                                  _infoRow('Metode Cicilan', _capitalize(transaksi!['metode_cicilan'])),
                                _infoRow('Tanggal Mulai', transaksi!['tanggal_mulai'].substring(0, 10)),
                                _infoRow('Tanggal Jatuh Tempo', transaksi!['tanggal_jatuh_tempo'].substring(0, 10)),
                              ],
                            ),
                          ],
                        ),
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  child: const Row(
                                    children: [
                                      Expanded(child: Center(child: Text('Tanggal Bayar', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)))),
                                      Expanded(child: Center(child: Text('Jumlah Dibayar', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)))),
                                      Expanded(child: Center(child: Text('Bukti Transfer', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)))),
                                    ],
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      final buktiUrl = item['bukti_transfer_url'];
                                      ...
                                      return Stack(
                                        children: [
                                          Container(
                                            ...
                                            child: Row(
                                              children: [
                                                Expanded(child: Center(child: Text(item['tanggal_bayar'].substring(0, 10)))),
                                                Expanded(child: Center(child: Text(item['jumlah']))),
                                                Expanded(
                                                  child: Column(
                                                    children: [
                                                      GestureDetector(
                                                              ...
                                                              builder: (context) => Dialog(
                                                                ...
                                                                child: InteractiveViewer(
                                                                  child: ClipRRect(
                                                                    ...
                                                                    child: Image.network(
                                                                      'http://localhost:3000/uploads/$buktiUrl',
                                                                      fit: BoxFit.contain,
                                                      ...
                                                      const SizedBox(height: 4),
                                                      const Text('Lihat Bukti', style: TextStyle(fontSize: 12)),
                                                    ],
                                                  ),
                                                ),
                                              ],
                                  ...
                                  child: ElevatedButton(
                                    onPressed: () async {
                                      await Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                          builder: (_) => FormCicilanScreen(transaksiId: widget.transaksiId),
                                        ),
                                      );
                                    ...
                                    child: const Text('Tambah Cicilan', style: TextStyle(fontSize: 16)),
                                  ),
                                ),
                              ],
                            ),
